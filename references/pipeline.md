...
| **실제 자산 배치 프로파일(scale 1.0)** | caster·coward·dangerbot·core_warden·blue_humanoid 전 행동 정상(잘림 0). ambusher death 1%·brute death 7% 미세(auto-fit 해결). → **실제 production 자산 대부분은 자연 상태(scale 1.0·기본 margin 1.3)에서 안 잘리고**, 무기 큰 일부만 특정 행동 미세 조정 필요(검사가 잡아 auto-fit 처리) |
