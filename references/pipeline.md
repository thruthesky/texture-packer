...
| **verify `--atlas` + auto-fit 안전** | packed atlas 근사 검사(`--atlas`): region 96개 파싱·여백 판정 동작(정밀은 `--frames`). auto-fit **무한루프 이중 방어** — `for _fit in range(_max_fit+1)`(최대 7회 렌더, `_max_fit=6`) + 0.6 하한 도달 시 `if not _changed: break`. **극단 실측**(bone `--margin 1.0` 여백 없음): attack/run 0.6 하한 도달 → 재렌더도 잔여 잘림 → "더 줄일 수 없음(0.6 하한) — margin 조정 필요" 안전 종료(무한루프 없음 + 근본 원인 margin 안내) |
...
| **실제 자산 배치 프로파일(scale 1.0)** | caster·coward·dangerbot·core_warden·blue_humanoid 전 행동 정상(잘림 0). ambusher death 1%·brute death 7% 미세(auto-fit 해결). **단 demonic·demonic_king(뿔·날개 큰 자산)은 자연 상태에서도 여러 행동 잘림**(0.85~0.92) → auto-fit 필요. **실측: demonic_king 여러 행동 잘림 auto-fit 1회 완전 수렴**(attack/death/hit/idle 1→0.88·run 1→0.85 → 재렌더 잘림 0 — 여러 행동 동시 잘림도 각각 다른 scale 로 한 번에 해결). → 실제 production 자산 대부분은 자연 상태(scale 1.0·기본 margin 1.3)에서 안 잘리나, 뿔·날개·무기 큰 자산은 검사가 잡아 auto-fit 처리. 배치 도구(`check_all_cells.sh`)에 진행 표시(N/총·예상 시간) 추가 |
| **최종 통합 회귀 스모크** | 전 스크립트(python 8·bash 2) syntax OK + 모든 수정(버그 3건: 프레임1 ZeroDiv·RGB false positive·auto-fit 조기종료 / 로직: step 하강·strip x/y 분리·Y trim offset 보정) 반영 후 ambusher end-to-end: 렌더 144장 → death auto-fit 0.93(최소 하강·회귀 없음) → 잘림 0 → packed atlas 생성 · `laryen.actionScale` 메타 6줄 · Y trim `offset y=7` 보정(총 21s). **모든 기능이 한 파이프라인에 정합** |
| **🔴 팀+자체 리뷰 결함 4건 수정** | ① **검사 타이밍(핵심)**: `align_frames_feet`(발 정렬)을 검사 *전*(렌더 직후)으로 이동 → verify_cells 가 **정렬 후 최종 packing 입력**을 검사. 과거엔 검사가 정렬 전이라 align 의 세로 이동으로 *새로 생기는 칼끝/무기 끝 잘림* 을 못 봤다(칼끝 잘림의 근본 원인). ② **auto-fit hard fail**: verify_cells 실패를 None 으로 구분해 `--auto-fit-scale` 이면 `sys.exit`(과거엔 {} 로 "잘림 없음"처럼 조용히 진행). ③ **--atlas 정보성화**: `size>=orig` 근사가 정상 자산(male_chrome 1024 region 전부)도 오탐 → exit 항상 0·"자동 판정 금지·--frames 로만 판정" 경고. ④ **sheet-win.py**: `DEFAULT_CELL_SIZE 160→128`(macOS SSOT 정합) + verify/auto-fit 이 macOS 전용임을 헤더에 명시(거짓 "100% 동일" 정정). 실측: ambusher 렌더→**정렬→검사**→auto-fit(attack 1→0.82·run 1→0.88)→재렌더→정렬→잘림 0 수렴. 🛑 **검증 중 부작용 발견·수정**: `align_frames_feet` 는 idempotent 가 *아니다*(원본 `_foot` 마스크 기준으로 매번 shift → 이미 정렬된 프레임에 재실행하면 이중 정렬로 하단 잘림 발생, `align_feet` 2회 실행 시 attack 22% 잘림 실측). 그래서 build-only 에 넣었던 align 을 *제거*(기존 낱장은 이전 렌더에서 이미 정렬됨). auto-fit 루프는 매 렌더가 새(정렬 안 된) 프레임이라 1회 정렬로 안전. 재검증: render-only 1종 잘림 = build-only 1종 잘림(동일 → 이중 정렬 없음 확인) |
| **--auto-fit-scale 이 --scale-<action> 무시** | 사용자 지시(2026-07-07): `--auto-fit-scale` 을 켜면 `--scale-<action>`·전역 `--scale` 을 *모두 무시* 하고 action_scales 를 **1.0(원본)** 으로 초기화한 뒤 auto-fit 이 잘림 기반으로 필요한 만큼만 하강한다(사람이 준 시작 scale 은 무의미하므로). 대화형 scale 질문도 건너뛴다(scale_actions=[]). 실측: `--auto-fit-scale --scale-attack 0.5 --scale-idle 0.5` → `↻ auto-fit --scale-attack 1→0.8`·`--scale-idle 1→0.8`(0.5 무시, 1.0 에서 시작). --help·SKILL.md(옵션 표·§3.5) 명시 |
| **🐛 auto-fit 수렴 실패(bone) 수정** | bone.blend 정밀 검사(실제 프레임) 시 attack/run top 잘림이 커서, 권장값(`1-max_frac-0.06`)이 재렌더로 잘림 줄면 *올라가* "권장>현재"로 조기 종료 → 잔여 4% 잘림. **수정**: `_target = max(0.6, min(권장, 현재-AUTOFIT_STEP 0.06))` step 하강 + `_max_fit 3→6`. 재검증: bone attack 1→0.83→0.77→0.71→0.65→**0.6**, run 1→0.84→0.78→**0.72** → 5회 만에 완전 잘림 0 수렴(이전엔 2회에서 멈춰 잔여). **회귀 없음**: 작은 잘림(ambusher death 1%)은 여전히 1회 최소 하강(0.93)으로 수렴 — step 하강은 잔여 잘림이 있을 때만이라 과잉 하강(0.6까지) 안 함 |
