...
| **🐛 auto-fit 수렴 실패(bone) 수정** | bone.blend 정밀 검사(실제 프레임) 시 attack/run top 잘림이 커서, 권장값(`1-max_frac-0.06`)이 재렌더로 잘림 줄면 *올라가* "권장>현재"로 조기 종료 → 잔여 4% 잘림. **수정**: `_target = max(0.6, min(권장, 현재-AUTOFIT_STEP 0.06))` step 하강 + `_max_fit 3→6`. 재검증: bone attack 1→0.83→0.77→0.71→0.65→**0.6**, run 1→0.84→0.78→**0.72** → 5회 만에 완전 잘림 0 수렴(이전엔 2회에서 멈춰 잔여) |
