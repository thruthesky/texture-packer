...
사용:  python3 align_feet.py <frames_dir> [foot_frac=0.85] [only_actions=a,b,c]

🛑 only_actions(3번째 인자, 콤마 구분): 주어지면 `{action}_*.png` prefix 가 그 행동에 속하는
낱장만 정렬한다. auto-fit 부분 재렌더에서 *새로 구운 행동만* 정렬하기 위한 것 — 이 정렬은
idempotent 가 아니라(canvas.paste 로 매번 shift) 이미 정렬된 다른 행동 프레임을 재정렬하면
이중 정렬로 하단이 잘린다. 재렌더된 행동만 정렬하면 보존된 행동은 이전 정렬 상태를 유지한다.
"""
import os
import sys
...
def align_dir(frames_dir, foot_frac=0.85, only_actions=None):
    ...
    used_mask = 0
    aligned = 0
    # only_actions 가 있으면 그 행동 prefix(`{action}_`)에 속하는 낱장만 정렬(부분 재렌더 대응).
    prefixes = tuple(f"{a}_" for a in only_actions) if only_actions else None
    for fn in sorted(os.listdir(frames_dir)):
        if not fn.endswith(".png"):
            continue
        if prefixes and not fn.startswith(prefixes):
            continue                              # 보존 대상(재렌더 안 한 행동) → 재정렬 금지
...
def main():
    if len(sys.argv) < 2:
        sys.exit("사용: align_feet.py <frames_dir> [foot_frac=0.85] [only_actions=a,b,c]")
    frames_dir = sys.argv[1]
    foot_frac = float(sys.argv[2]) if len(sys.argv) > 2 else 0.85
    only_actions = ([a for a in sys.argv[3].split(",") if a.strip()]
                    if len(sys.argv) > 3 and sys.argv[3].strip() else None)
    if not os.path.isdir(frames_dir):
        sys.exit(f"frames 폴더가 없습니다: {frames_dir}")
    n, used = align_dir(frames_dir, foot_frac, only_actions)
    scope = f", 대상={','.join(only_actions)}" if only_actions else ""
    print(f"aligned {n} (foot_frac={foot_frac}, 마스크 기준={used}, bbox 폴백={n - used}{scope})")
