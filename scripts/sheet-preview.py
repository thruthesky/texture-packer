...
Usage examples:
  ...
  py scripts\sheet-preview.py game-assets\characters\mob\chassis\chassis.blend

  # Spelled out (still supported — explicit flags always win over inference)
  ./scripts/sheet-preview.py --character game-assets/characters/pc/male/male/male.blend \
...
Preview only specific actions (skip rendering the whole set):
  ...
  Example — attack only:
    ./scripts/sheet-preview.py --character game-assets/characters/male.blend --name male \
...
_PREVIEW_BANNER = (
    "# AUTO-GENERATED by sheet-preview.py — DO NOT EDIT.\n"
    "# This is a copy of production _sheet_render.py / _sheet_build.py + a 4-direction(preview) patch,\n"
    "# regenerated from the originals on every sheet-preview.py run (the originals are the SSOT). Edit the originals.\n"
)
...
def _require_replaced(src, what, variants, count=-1):
    ...
    raise SystemExit(
        f"sheet-preview.py: could not patch the production render/build helper for '{what}'.\n"
        f"   The production _sheet_render.py / _sheet_build.py wording changed and no known variant\n"
        f"   matched. Update the variants list in sheet-preview.py:_ensure_preview_helpers() to the\n"
...
def assert_mixamo_rig(path, role):
    ...
    if b"mixamorig" not in blob:
        sys.exit(
            ...
            f"   -> sheet-preview.py only supports a *Mixamo-rig character + Mixamo animations*.\n"
...
def main():
    ...
    ap.add_argument("--vivid", type=int, default=9, choices=range(1, 10), metavar="1-9",
                    help="Color intensity (contrast) + brightness strength (1-9, default 9, same as "
                         "sheet.py). 9=max bright/vivid, 5=medium, 1=no boost. Applied via "
    ...
    ap.add_argument("--auto", action="store_true",
                    help="Mirror sheet-win.py's production preset for a side-by-side compare: --pack + "
                         "--auto-fit-scale + --color-compression true (vivid 9 / eevee are already the preview "
