...
# <1 이면 그 행동 셀을 1/scale 로 키워(무기/모션이 128 셀 밖으로 나가는 잘림 방지·body 화질 유지) `.atlas` 헤더
        ...
        for _fit in range(_max_fit + 1):
            ...
            for _a, _s in _rec.items():
                ...
                if _target < _cur - 1e-6:
                    ...
                    print(f"   ↻ auto-fit: --scale-{_a} {_cur:g}→{_target:g} (셀 ×{1.0/_target:.3f}={round(128/_target)}px)")
                    _changed = True
