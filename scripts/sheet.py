...
# 행동별 *생성 scale* 기본값(2026-07-07 사용자 지시). <1 이면 그 행동 모델을 작게 구워(무기/모션이
# 128 셀 밖으로 나가는 잘림 방지) `.atlas` 헤더 `laryen.actionScale.<action>` 메타에 기록하고,
# 게임 런타임(actor_animation_set.dart parseDisplayScales)이 표시 배율 1/scale 로 *원래 크기 복원* 한다.
# attack 은 여기 두지 않는다 — finalize 의 대화형 질문(기본 0.8)이 처리(미지정 시 개발자에게 물음).
# look/talk/wave(npc)도 미포함 — 전역 --scale fallback. 여기 없는 행동은 default None → 전역 --scale.
SCALE_DEFAULTS = {"idle": 1.0, "walk": 0.9, "run": 0.9, "hit": 0.9, "death": 1.0}
TEXTURE_LIMIT = 8192
...
def main():
    ...
    for a in FRAME_OPTION_ACTIONS:
        _sd = SCALE_DEFAULTS.get(a)
        ap.add_argument(f"--scale-{a}", type=float, default=_sd, dest=f"scale_{a}",
                        help=f"{a} 행동 생성 scale"
                             + (f"(기본 {_sd:g}, <1=모델 작게 구워 런타임 1/scale 복원)" if _sd is not None
                                else "(미지정=전역 --scale)")
                             + ". atlas 메타로 기록돼 게임에서 원래 크기로 복원됨.")
