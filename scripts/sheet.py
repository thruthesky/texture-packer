...
# → 가장 무거운 attack(16→10)과 화면에 잠깐만 보이는 death(8→6)를 먼저 깎고, 상시 노출되는
#   idle 은 8 그대로 둔다(정지 중 몬스터가 뚝뚝 끊겨 보이면 가장 눈에 띈다).
# 🛑 축소 디코드(저사양 60% 등)와 결정적으로 다른 점: **화질 손실이 0** 이다. 픽셀은 그대로 두고
...
def prompt_missing(args):
    ...
    if args.kind in ("pc", "mob") and not args.animations:
        ...
        if _anim_status == "empty" and not args.animations:
            ...
            sys.exit(
                f"❌ 애니 폴더가 비어 있습니다: {ANIM_ROOT}/{args.name}/\n"
                f"   game-assets/animations/{args.name}/ 폴더는 존재하지만 애니 파일"
                f"(.fbx/.glb/.gltf)이 하나도 없습니다.\n"
                f"   -> 애니 파일을 넣거나, --animations <name> 으로 다른 variant 를 지정하세요"
                f"(예: --animations default).")
        if name_anim:
...
def main():
    ...
    ap.add_argument("--actions", default=None,
                    help="행동 순서/목록. 기본 pc/mob=idle,walk,attack,death,run; npc=idle,look,talk,walk,wave")
