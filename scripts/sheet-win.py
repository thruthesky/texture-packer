...
출력 경로(--kind + --name):
  ...
  --res           예비(reserve) 보관 출력. assets/<kind>/<name>/ 대신
                  game-assets/sprites_reserve/<name>/<name>.{png,atlas} 로 저장한다(<kind> 하위폴더
                  없음). 앱 번들에 포함되지 않는 keep-only 폴더라 pubspec.yaml 자동갱신을 건너뛴다.
                  --output 보다 우선.
...
def main():
    ...
    ap.add_argument("--res", action="store_true",
                    help="Reserve output — save the resulting atlas/png to game-assets/sprites_reserve/<name>/ "
                         "(no <kind> subfolder) instead of assets/<kind>/<name>/. Like --output, the root "
                         "pubspec.yaml auto-update is skipped (the reserve folder is a keep-only store, not bundled "
                         "into the app). Takes priority over --output when both are given.")
    ...
    # --res 지정 → 예비(reserve) 보관 폴더 game-assets/sprites_reserve/<name>/ 로 저장한다.
    #   앱 번들에 포함되지 않는 keep-only 폴더라 <kind> 하위폴더 없이 <name>/ 바로 아래에 넣고,
    #   아래 pubspec 자동갱신도 --output 과 동일하게 건너뛴다. --output 보다 우선한다.
    ...
    if args.res:
        out_folder = os.path.join(ROOT, "game-assets", "sprites_reserve", name)
    else:
        if args.output_base:
