...
출력 경로(--kind + --name):
  --kind {pc,mob,npc,boss,minion}
                       액터 카테고리(pc=플레이어, mob=몬스터, npc=마을 NPC,
                       boss=보스 8방향, minion=졸개 8방향·64cell·화면 절반).
  ...
  --res           예비(reserve) 보관 출력. assets/<kind>/<name>/ 대신
                  game-assets/sprites_reserve/<kind>/<name>/<name>.{png,atlas} 로 저장한다.
                  앱 번들에 포함되지 않는 keep-only 폴더라 pubspec.yaml 자동갱신을 건너뛴다.
                  --output 보다 우선.
...
# FLARE16 방향 라벨 SSOT — `_sheet_render.py` DIR16_LABELS · 런타임 kDir16Labels 와 동일 순서.
# 8방향 sheet 는 이 배열의 *짝수* 인덱스만 쓴다(E,SE,S,SW,W,NW,N,NE).
DIR16_LABELS_SSOT = ["E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW",
                     "W", "WNW", "NW", "NNW", "N", "NNE", "NE", "ENE"]

# ── 액터 kind 정책 SSOT (2026-07-27) — sheet.py 와 **동일 테이블**(Windows 형제 파일 동기화) ──
# kind 별 "방향 수·cell·화면 표시 크기·기본 행동·모델 폴더"를 한 테이블로 모은다. 새 kind 는
...
def infer_kind_name_from_path(path):
    ...
    구조를 못 알아보면 **조용히 추측하지 않는다** — 기존 흐름(대화형/에러)으로 넘긴다.
    잘못 추론해 엉뚱한 kind 로 구우면 방향 수·cell 이 달라져 자산이 통째로 어긋난다.
    """
    try:
    ...
    for i in range(len(parts) - 2, -1, -1):
        ...
        if len(rest) < 3:
            return None, None             # 최소 kind/name/file
        kind = rest[0]
        if kind not in KIND_POLICY:
            return None, None
        # pc 는 성별 단계가 하나 더 있다: pc/<gender>/<name>/<file>
        if kind == "pc":
            if len(rest) < 4:
                return None, None
            name = rest[-2]
...
def list_models(kind):
    """대화형 모델 목록. 평면(<dir>/<name>.blend)과 중첩(<dir>/<name>/<name>.blend) 둘 다 훑는다.

    🛑 2026-07-28: 폴더 구조가 `<kind>/<name>/<name>.blend` 중첩으로 재편(a0c03f26)됐는데
    이 함수는 `os.listdir` 로 한 겹만 봐서 **항상 빈 목록** 이었다(대화형 선택 불가).
    pc 는 `pc/<gender>/<name>/<name>.blend` 라 한 단계 더 들어간다.
    반환값은 KIND_MODEL_DIR 기준 *상대 경로* 라 기존 상대경로 보정과 그대로 맞물린다."""
    base = os.path.join(ROOT, KIND_MODEL_DIR.get(kind, ""))
    if not os.path.isdir(base):
        return []
    out = []
    # 평면 — <base>/<name>.blend (구 구조 잔존 파일)
    for f in sorted(os.listdir(base)):
        if os.path.splitext(f)[1].lower() in CHAR_EXT:
            out.append(f)
    # 중첩 — <base>/[<gender>/]<name>/<name>.blend
    depth = 2 if kind == "pc" else 1        # pc 만 성별 단계가 하나 더
...
def resolve_actor_model(path, kind, role, build_only=False, announce_blend=True):
    ...
    if not os.path.isfile(path):
        ...
        if os.path.isfile(cand):
            ...
            if alt:
                print(f"  ℹ️  '{path}' not found -> auto-corrected extension: {alt}")
                path = alt
            else:
    ...
    if ext not in CHAR_EXT:
        sys.exit(f"Unsupported {role} format: {ext or '(no extension)'} — {path}")
    if ext == ".blend":
        if announce_blend:
...
def prompt_missing(args):
    ...
    if not args.kind:
        if not interactive:
            sys.exit(f"--kind {{{','.join(ALL_KINDS)}}} is required (not interactive — not a terminal).")
        args.kind = _choose("Pick an actor category (--kind):", list(ALL_KINDS), allow_manual=False)
    ...
    #       꺾이는 retarget 회귀가 난다(asset ssot.md §retarget).
    #    ⚠️ 아래 "이름 폴더 우선" 을 없애지 말 것 — game-assets/animations/ 에는 모델 이름과
    #       일치하는 전용 세트가 49개나 있어(2026-07-28 실측), 이 단계를 건너뛰고 곧바로
    #       default 로 가면 그 49개가 통째로 무시된다.
    ...
    if args.kind in ("pc", "mob") and not args.animations:
    ...
    #    🛑 With --auto-fit-scale we do not ask at all: an explicit --scale-<action> becomes the
    #    starting scale and the rest start at 1.0, with auto-fit lowering them as needed.
...
def inject_action_scales(atlas_path, action_scales, directions=None, display_size=None):
    ...
    # 방향 수·종족 표시 크기는 기본값(16·128)과 다를 때만 기록 — 기존 자산은 메타 없이 fallback.
    extra = []
...
def validate_atlas(atlas_path, kind, directions=None):
    ...
    # 🛑 런타임 getDir16 의 최종 fallback 은 `_table[idle][dir]!` 라 idle 이 한 방향이라도 비면
    # 그 방향에서 null assertion 크래시. 8방향 접미사는 FLARE16 *짝수* 라벨이어야 런타임
    # nearest8FromDir16(2k→k) 과 정합한다.
    if directions in (8, 16):
        want = set(DIR16_LABELS_SSOT if directions == 16 else DIR16_LABELS_SSOT[::2])
        got_idle, bad_labels = set(), set()
        for r in regions:
            nm = r["name"]
            if "_" not in nm:
                continue
        ...
        if bad_labels:
            problems.append(
                f"{directions}-dir sheet has {len(bad_labels)} out-of-spec direction labels: "
                f"{', '.join(sorted(bad_labels))} (8-dir allows FLARE16 even labels only)")
        missing_idle = want - got_idle
        if missing_idle:
            problems.append(
                f"idle region missing for {len(missing_idle)} directions: "
                f"{', '.join(sorted(missing_idle))} — runtime fallback (_table[idle][dir]!) will CRASH")
...
def main():
    ...
    # 위치 인자 — `sheet.py <모델경로>` 한 줄 실행(2026-07-28).
    # 경로에 kind·name 이 이미 들어 있고 애니메이션도 그 폴더에 있으므로, 그것만 주면
    ...
    for _act in FRAME_OPTION_ACTIONS:
        ap.add_argument(f"--character-{_act}", f"--actor-{_act}", dest=f"character_{_act}", default=None,
                        help=f"Render only the '{_act}' action from this model (default: --character). "
                             f"Same formats as --character; overrides the auto-detected "
                             f"<model>_{_act}.<ext> sibling.")
    ap.add_argument("--action-models", dest="action_models", type=str2bool, nargs="?", const=True,
                    ...
                    help="Auto-use per-action sibling models: <model_stem>_<action>.<ext> next to "
                         "--character is rendered for that action (e.g. male_claudean_attack.blend -> "
                         "attack). Default true. Animation files in that folder are named "
                         "<action>.fbx (no stem prefix) so they are never mistaken for models.")
    ...
    for a in FRAME_OPTION_ACTIONS:
        ...
        ap.add_argument(f"--scale-{a}", type=float, default=None, dest=f"scale_{a}",
                        help=f"{a} action generation scale"
                             ...
                             + "🛑 With --auto-fit-scale this value is the *starting* scale (actions without it start at 1.0) "
                             + "and auto-fit only lowers it further if clipping remains.")
    ...
    ap.add_argument("--res", action="store_true",
                    help="Reserve output — save the resulting atlas/png to game-assets/sprites_reserve/<kind>/<name>/ "
                         "instead of assets/<kind>/<name>/. Like --output, the root pubspec.yaml auto-update is "
                         "skipped (the reserve folder is a keep-only store, not bundled into the app). Takes priority "
                         "over --output when both are given.")
    ...
    ap.add_argument("--auto", action="store_true",
                    help="🚀 One-click optimal preset — turns on `--texture-pack true --auto-fit-scale "
                         ...
                         "false` turns off only rotation and keeps the rest of the preset. Since auto-fit is enabled, each "
                         "--scale-<action> you pass becomes that action's *starting* scale (others start at 1.0) and auto-fit "
                         "lowers it from there; the global --scale is ignored.")
    ap.add_argument("--auto-fit-scale", dest="auto_fit_scale", action="store_true",
                    help="When a clipped action is found, lower the scale and *auto re-render* (up to 6 iterations · 0.667 floor = cell 1.5x/192, converging "
                         ...
                         "🛑 Start point: an action given --scale-<action> starts from that value, every other action starts from "
                         "1.0 (original); the global --scale is ignored. auto-fit only decreases from there, as much as it needs "
                         "(the interactive scale prompt is also skipped).")
    # ── TexturePacker 전용 ──
    ...
    # ── --blend <name>: --character game-assets/characters/<name>.blend 의 단축 표기 ──
    ...
    # ── 경로 한 줄 실행: kind·name·애니·--auto 자동 채움 (2026-07-28) ──────────────
    #
    # 🛑 반드시 아래 `--auto` 프리셋 블록보다 **먼저** 실행한다 — auto 는 kind 를 보고
    # 동작을 바꾸므로(mob 계열이면 run 애니 제외 등) kind 가 정해진 뒤여야 한다.
    ...
    if args.model:
        ...
        if ik:
            ...
            if not args.name:
                args.name = iname
                print(f"  ✓ 경로에서 name 추론: {iname}")
                stem = os.path.splitext(os.path.basename(args.character))[0]
                if stem != iname:
    ...
    # 🛑 boss/minion 의 8방향은 "신규는 16방향" 원칙의 *명시적 예외*(2026-07-27)이므로 legacy 경고 없음.
    fixed = FIXED_DIRECTION_KINDS.get(args.kind)
    if fixed is not None:
        if directions != fixed:
            sys.exit(f"--kind {args.kind} is fixed to {fixed} directions "
                     f"(--directions {fixed} or omit). got: {directions}")
        if args.kind == "npc" and actions != NPC_ACTIONS:
            sys.exit("--kind npc supports only the single idle animation.")
    elif directions == 8:
        print("  ⚠️  --directions 8 is for regenerating legacy PC/mob sheets "
              "(new PC/mob use 16 directions; use --kind boss|minion for 8-direction assets).")
    if args.kind == "minion" and cell != policy["cell"]:
        print(f"  ⚠️  --kind minion standard cell is {policy['cell']} (got {cell}). "
              f"On-screen size is recorded as laryen.displaySize={policy['display']}.")
    name = args.name
    ...
    for a in actions:
        if args.auto_fit_scale:
            ...
            # 🛑 Keep identical to sheet.py — a one-sided edit silently reverts on that OS.
    ...
    if args.auto_fit_scale:
        ...
        if _given:
            print("  ℹ️  auto-fit start scale (from --scale-<action>): "
                  + " · ".join(f"{a}={s:g}" for a, s in _given.items())
                  + " — other actions start at 1.0")
    if args.render_res:
    ...
    # --res 지정 → 예비(reserve) 보관 폴더 game-assets/sprites_reserve/<kind>/<name>/ 로 저장한다.
    #   앱 번들에 포함되지 않는 keep-only 폴더라 아래 pubspec 자동갱신을 --output 과 동일하게
    #   건너뛴다. --output 보다 우선한다.
    ...
    if args.res:
        out_folder = os.path.join(ROOT, "game-assets", "sprites_reserve", args.kind, name)
    else:
        if args.output_base:
    ...
    for _a in actions:      # 행동별 모델이 배정된 행동만 한 줄씩(없으면 아무 줄도 안 나온다)
        if os.path.abspath(action_character[_a]) != os.path.abspath(args.character):
            print(f"   └ action    : {_a:<7} -> {action_character[_a]}  (per-action model)")
    ...
    if not args.build_only:
        ...
        def _run_blender(pass_cfg_path, pass_frames, tag=""):
            ...
            for line in proc.stdout:
                ...
                if line.startswith("####ACTION "):
                    ...
                    if saved % 24 == 0 or saved >= pass_frames:
                        ...
                        print(f"   … {saved}/{pass_frames} ({pct}%) · {fps:.1f} frames/s · ETA {_fmt_dur(eta)}{tail}",
                              flush=True)
            ...
            if proc.returncode == 0 and not render_done and on_disk >= total_frames:
                print(f"   ⚠️ RENDER_DONE marker missing but frames {on_disk}/{total_frames} complete → proceeding")
                render_done = True
            if proc.returncode != 0 or not render_done:
                print(f"   ❌ render failed{tag} — check input FBX / Blender log:")
                for e in errs[-20:]:
                    print("     " + e)
                sys.exit("render failed")
            print(f"   ✓ render complete{tag} — {saved} frames (on disk {on_disk}) · {_fmt_dur(_r_dt)}"
                  + (f" · {saved / _r_dt:.1f} frames/s" if _r_dt > 0 else ""))
        ...
        for _fit in range(_max_fit + 1):
            ...
            if _fit == 0:
                ...
                if multi_model:
                    print(f"    per-action models — {len(groups)} Blender passes: "
                ...
                _scope = (f"clipped actions only: {', '.join(_render_only)} ({_fit_frames} frames)"
    ...
    if args.output_base or args.res:
        why = "--res (reserve store)" if args.res else "--output"
        print(f"\n[pubspec] {why} given -> skipping root pubspec.yaml auto-update. "
