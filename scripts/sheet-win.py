...
Applying to the game:
  1. To replace an existing PC/monster, generate with the same --name as the existing kind.
     e.g. --kind pc --name male_victor, --kind mob --name brute.
...
Examples (PowerShell — line continuation is backtick `):
  ...
  # packed atlas (default) — PC male_victor
  py scripts\sheet-win.py --kind pc --name male_victor `
    --character game-assets\characters\male_victor.fbx --animations default `
    --idle 8 --walk 12 --run 12 --attack 16 --hit 8 --death 8
...
def prompt_missing(args):
    ...
    if not args.animations:
        ...
        if not interactive:
            sys.exit("--animations is required (not interactive).")
        args.animations = _choose(f"Pick an animation folder (--animations, {ANIM_ROOT}/):",
                                   variants)
...
def main():
    ...
    ap.add_argument("--name", default=None,
                    help="Output texture filename (e.g. male_victor). → assets/<kind>/<name>/<name>.{png,atlas}")
    ap.add_argument("--animations", default=None,
                    help=f"Animation variant folder name or path ({ANIM_ROOT}/<name>). If omitted, interactive list selection. "
    ...
    for a in FRAME_OPTION_ACTIONS:
        ...
        ap.add_argument(f"--scale-{a}", type=float, default=None, dest=f"scale_{a}",
                        help=f"{a} action generation scale"
                             ...
                             + ". <1=bake the model smaller and record atlas meta → game runtime restores original size by 1/scale. "
    ...
    if args.kind == "npc":
        if directions != 8:
            sys.exit("--kind npc supports 8 directions only. Generate with --directions 8.")
        if actions != NPC_ACTIONS:
            sys.exit("--kind npc supports only the 5 animations idle,look,talk,walk,wave.")
    elif directions == 8:
    ...
    print(f"  anim folder : {animations_dir}  ({', '.join(actions)})")
    if args.weapon:
